...
import { tweakSigner, Uint8ArrayToHex } from "../utils";
...
export class SimpleKeyring extends EventEmitter {
  ...
  async serialize(): Promise<string[]> {
    return this.wallets.map((wallet) =>
      Uint8ArrayToHex(wallet.privateKey ?? Buffer.alloc(0))
    );
  }

  async deserialize(opts: string[] | ECPairInterface[]) {
    ...
    this.wallets = privateKeys.map((key) => {
      ...
      if (key.length === 64) {
        ...
        // base58
        buf = Buffer.from(bs58check.decode(key).slice(1, 33));
      }
  ...
  async getAccounts() {
    return this.wallets.map(({ publicKey }) => Uint8ArrayToHex(publicKey));
  }
  ...
  async signData(
    ...
    const keyPair = this._getPrivateKeyFor(publicKey);
    if (type === "ecdsa") {
      return Uint8ArrayToHex(keyPair.sign(Buffer.from(data, "hex")));
    } else if (type === "schnorr") {
      return Uint8ArrayToHex(keyPair.signSchnorr(Buffer.from(data, "hex")));
    } else {
  ...
  async exportAccount(publicKey: string) {
    ...
    if (!wallet.privateKey) {
      throw new Error("Private key not found on Account");
    }

    return Uint8ArrayToHex(wallet.privateKey);
  }

  removeAccount(publicKey: string) {
    if (
      !this.wallets
        .map((wallet) => Uint8ArrayToHex(wallet.publicKey))
        .includes(publicKey)
    ) {
    ...
    this.wallets = this.wallets.filter(
      (wallet) => Uint8ArrayToHex(wallet.publicKey) !== publicKey
    );
  }
